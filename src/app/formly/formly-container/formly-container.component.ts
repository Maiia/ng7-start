...
export class FormlyComponent implements OnChanges, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    const jsonToChange = this.fields.filter(item => item.type === 'radio' || item.type === 'multicheckbox' || item.type === 'checkbox');
    ...
    this.formContainer.nativeElement.querySelectorAll('.mat-radio-label-content, .mat-checkbox-label').forEach(
      (item, index) => {
        if (imagesArr[index]['type'] === 'image') {
          item.innerHTML = converter.makeHtml(`![Alt text](${imagesArr[index]['label']})`)
        }
      }
    );
  }
